...
local function directives()
	...
	local directives_list = vim.treesitter.query.list_directives()
	if not tbl.contains(directives_list, "inject_without_named_children!") then
	...
	if not tbl.contains(directives_list, "inject_without_children!") then
...
vim.notify("styled-components syntax highlighting loaded", "info", { title = "styled-components" })
-- directives()
-- queries()
